...
function apiShow() {
    ...
    $('.api-container .top .text p span:first-child').on('click', function () {
       if(flagOne){
           ...
           //点击时隐藏另外一个弹出框
           if($('.api-container .type').css('display')==='block'){
    ...
    $('.api-container .top .text p span:nth-child(3)').on('click', function () {
        if(flagTwo){
            ...
            //点击时隐藏另外一个弹出框
            if($('.api-container .msg-voi').css('display')==='block'){
                $('.api-container .top .text p span:nth-child(1)').trigger('click');
            }
            flagTwo =false;
        }else{
...
function sdkShow() {
    ...
    $('.sdk-container .top .text p span:first-child').on('click', function () {
        if(flagOne){
            ...
            //点击时隐藏另外一个弹出框
            if($('.sdk-container .type').css('display')==='block'){
    ...
    $('.sdk-container .top .text p span:nth-child(3)').on('click', function () {
        if(flagTwo){
            ...
            //点击时隐藏另外一个弹出框
            if($('.sdk-container .msg-voi').css('display')==='block'){
                $('.sdk-container .top .text p span:nth-child(1)').trigger('click');
            }
            flagTwo =false;
...
function apiChoose(){
    ...
    $('.api-container .msg-voi ul').on('click','li', function () {
        ...
        // 当选择语音时，显示type-voi
        if(_index===1){
            $('.api-container .type .type-msg').css('display','none');
            $('.api-container .type .type-voi').css('display','block');
        }else{
            $('.api-container .type .type-msg').css('display','block');
            $('.api-container .type .type-voi').css('display','none');
        }
        ...
        if(valueOne==='短信'&&valueTwo==='模版发送接口'){
            $('.api-container .top .look').css('background-color','#bbb')
        };
        if(valueOne==='语音'&&valueTwo==='单条发送接口'){
            $('.api-container .top .look').css('background-color','#bbb')
        };
        if(valueOne==='语音'&&valueTwo==='相同内容群发接口'){
            $('.api-container .top .look').css('background-color','#bbb')
        };
        if(valueOne==='语音'&&valueTwo==='个性化群发接口'){
            $('.api-container .top .look').css('background-color','#bbb')
        };
        if(valueOne==='语音'&&valueTwo==='获取上行接口'){
            $('.api-container .top .look').css('background-color','#bbb')
        };
    ...
    function apiToDetails(){
        //跳转并传真方法
        function jumpVal(){
            var  url = "API-common-details.html?valueOne="+valueOne+"&valueTwo="+valueTwo;//此处拼接内容传值
...
function sdkChoose(){
    ...
    $('.sdk-container .msg-voi ul').on('click','li', function () {
        ...
        if(valueOne=='PHP'&&valueTwo=='实例化短信对象'){
            $('.sdk-container .top .look').css('background-color','#bbb')
        };
